...
包中只保留打包脚本显式列出的正式Python源码、ECharts资源及第三方通知、当前技术/操作文档、部署和空配置示例、依赖清单、经过静态筛选的测试、测试隔离文件及 `.github/workflows/ci.yml`。精确文件数与逐文件哈希见同包清单。

当前增量见[28项整改与验收指南](docs/28项整改与验收指南.md)：会计方向约束、人工因素桥接、小比例精度、三要素配对下钻、图表联动、引用卡片、领域图谱、报告阅读版及独立业务验收均有对应代码与验收入口。历史审查和旧报告不会被重写为新的通过结果。
...
local模式是当前OS用户全角色演示，只允许本机回环访问，审批为模拟操作。整改业务验收仍要求独立人员，本机单用户不能自行关闭本人提交的整改；企业部署应配置OIDC与服务端角色、工厂/产品范围映射，不应扩大local绑定范围作为替代。

模块四默认向官方8090 loopback mock发送，可信配置可显式列入其他测试回环端口，不能发送真实微信。官方mock源码未包含，需由用户从授权资料另行提供；`COST_OFFICIAL_MOCK_SCRIPT`指向该脚本后，启动器的 `--with-mock`才可使用。批准、签发、发送受理、模拟执行完成与本地业务验收关闭分别记录。任务发送不明结果按原task_id查询；独立催办接口没有远端查询能力，结果不明时保留待核对，禁止更换ID或重复POST绕过保护。
...
本包只附逐文件审查的公开测试白名单，不等同原项目全量测试。完整清单由 `scripts/build_source_bundle.py` 的 `portable_test_paths()` 返回，并记录于源码包manifest。以下为部分测试及其边界，不是完整文件清单：
...
- 后续公开白名单还包括受控行业SIMULATION、金额与证据护栏、报告导出、阅读视图、源码打包，以及本轮 `test_round3_narrative.py`、`test_round3_model_selection.py`、`test_round3_presentation.py`。实际文件范围以构建器函数及manifest为准，不以本段示例数量推断。
