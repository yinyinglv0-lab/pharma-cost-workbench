...
旧无标记单品归因（M2）使用`M2_INSTRUCTION`，旧提示版本`attribution-hypotheses/1.3`（新版受控散文见4.1），输出三个要素各自的`hypothesis/recommendation/evidence_ids`。首次对象回复业务校验失败时，默认模型worker仅在同一45秒总预算和剩余时间条件下允许一次校验反馈修订，不能因认证/网络/网关JSON失败重试；父进程再次校验候选。`model_run.attempts`分别保留初次与修订的状态、指令/输入/回复hash、诊断、耗时与采用标记，修订合格不抹去首败。测试注入回调单次执行，不替代正式可终止worker的运行证据。合成PID测试检查执行前实际进程存活与超时回收后的退出；Windows和Linux覆盖范围以各自记录为准。网关将None/空白/非文本响应作为协议失败，不补成空对象触发修订。
...
### 4.1 2026-09-24分析文本与模型选择更新

新版本M2/M3采用 `1.4-bound-numeric-compact` 提示词和同一严格数字/证据/因果校验：重点材料参考价的变化只作条件式价格线索，不代表实购价；正文改为主要/次要会计来源，详细桥接仍留存；固定工时比率变化不推断岗位效率；对标将差异与原因分开，明细余额仅由实际剩余明细合计，拆结构与拆原因避免全量数字复述。渲染器统一黑色小节标题和蓝色建议标签、左线，不要求模型输出HTML。

设置页允许有权管理员在服务端注册的模型中选择，作用于全部署的新M2/M3及同口径报告。页面不接收Key/地址，不自动探测，也不改变外发授权。新注册模型按1至120秒的配置请求时限及 `min(255,2×时限+15)` 阶段预算执行；原默认配置仍保持新散文95/旧合同45秒与单次40秒上限。父子进程核对安全配置摘要，模型选择、请求/返回模型和实际调用摘要单独留痕；版本未返回则不推测。详见 `docs/任务级模型配置.md`、`docs/受控散文生成与阅读导出.md`。上节45秒描述仅对应旧合同，不能套用于新增注册配置。

产品成本分析默认金额分解图明确标注“Y轴截断以突出变动量”，保留全值金额、数据点级合计标签和累计台阶；另有真正零基线差额图。两个视图读取同一 `amount_change` 行，不改数值或贡献口径。

### 4.2 跨行业可迁移性实证

主界面专注制药赛题，已移除“制造业配置与分析”导航；通用服务、API和历史冻结分析保留。声明式配置不是权限，不会自动加载任意代码、覆盖生产知识或发送任务。

五组SIMULATION行业（制药、机械、汽配、化工、电子）曾实际运行同一五表接入、BGE知识发布、归因/对标、10份DOCX/PDF和官方进程内mock链。**2276项程序检查通过的批次，真实模型采纳数仍为0**；随后另一个模型明确关闭批次有2256项确定性/导出/mock检查通过。这两个历史批次不能相加或倒签为当前新版本实模通过，更不是五个真实企业部署。完整范围、源码标识和内部回执位置见 `docs/跨行业迁移与边界.md` 的“跨行业可迁移性实证”一节。保留实证而移除导航，不等于删除可迁移能力。
